...
const FName AVehiclePawn::LookUpBinding("LookUp");
const FName AVehiclePawn::LookRightBinding("LookRight");
...
void AVehiclePawn::OnToggleCamera()
{
	EnableIncarView(!bInCarCameraActive);
}

void AVehiclePawn::EnableIncarView(const bool bState, const bool bForce)
{
...
void AVehiclePawn::Tick(float Delta)
{
	...
	if (GetActorUpVector().Z <= KINDA_SMALL_NUMBER)
	{
		FTransform currentTransform = GetActorTransform();
		FTransform targetTransform = FTransform{ FRotator{0.0, currentTransform.Rotator().Yaw, 0.0}, currentTransform.GetTranslation()};
		SetActorTransform(targetTransform, false, nullptr, ETeleportType::TeleportPhysics);
		VehicleMovement->UpdatedPrimitive->SetPhysicsLinearVelocity(FVector{ 0, 0, 0 });
		VehicleMovement->UpdatedPrimitive->SetPhysicsAngularVelocityInDegrees(FVector{ 0, 0, 0 });
	}
}
...
void AVehiclePawn::SetupInCarHUD()
{
